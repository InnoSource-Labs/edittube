...
const Workspace = (): React.ReactNode => {
    ...
    const { workspace, filter, setFilter, videos, getMoreVideos, id, totalPage, currentPage } = useWorkspace();

    return (
        <div className='flex items-center justify-center flex-col md:w-[80vw] w-[95vw] m-auto' id="videosdiv">
            <div>Workspace</div>
            <div className='text-3xl font-semibold my-4 truncate w-full text-center'>{workspace?.name}</div>
                ...
                <div className='flex justify-between items-center w-full'>
                    ...
                    <FormControl sx={{ m: 1, minWidth: 100 }} size="small">
                        <Select
                            value={filter}
                            onChange={(e) =>
                                setFilter(e.target.value as VideoFilters)
                            }
                        ...
                        <div className='w-full'>
                            ...
                            >
                                {videos.map((video, index) => {
                                    return (<Link to={`/workspace/${id}/${video._id}`} key={index}>
